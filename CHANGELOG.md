...
* Added `check_data_is_not_empty` to detect empty `.data` fields in NWB data containers (TimeSeries, ElectricalSeries, etc.), which often indicate incomplete data entry or conversion errors. Skips ImageSeries with external_file set, where empty data is intentional. [#668](https://github.com/NeurodataWithoutBorders/nwbinspector/pull/668)
