...
def test_false_positive_skip_check_image_series_data_size():
    ...
    two_photon_series = TwoPhotonSeries(
        name="TwoPhotonSeries",
        imaging_plane=imaging_plane,
        data=np.empty((21e9, 1, 1)),  # Empty data, but of shape+dtype that would be more than default GB threshold
        unit="n.a.",
        rate=30.0,
    )

    assert check_image_series_data_size(image_series=two_photon_series) is None
